...
// Syscall tao file
void SysCreate(char *filename)
{
    if (strlen(filename) == 0)
    {
        printf("File name is not valid\n");
        DEBUG('u', "File name is not valid\n");
...
first implement for one parameter argv[1]

 Input: argc, argv
 call Exec
 */
//  void SysExecV(int argc, char* argv[]){
 ...
 */
 ...
 /*
 xu ly syscall remove
 */

 int SysRemove(char* filename){
    if(filename == NULL || strlen(filename) == 0){
        DEBUG(dbgSys,"filename error\n");
        return -1;
    }

    if(!kernel->fileSystem->Remove(filename)){
        DEBUG(dbgSys,"fail remove\n");
        return -1;
    }
    DEBUG(dbgSys,"remove success\n");
    return 0;
 }
